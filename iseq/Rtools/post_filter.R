...
option_list <- list(make_option(c("-v", "--verbose"), action = "store_true", default = TRUE, 
  help = "Print extra output [default]"), make_option(c("-i", "--inputcsvs"), help = "input csv"), 
  make_option(c("--min-casedepth"), dest = "mincasedepth", help = "Mininm case total depth", 
    default = 0), make_option(c("--min-casefrq"), dest = "mincasefrq", help = "Mininm case mutation frquency", 
    default = 0), make_option(c("--max-controlfrq"), dest = "maxcontrolfrq", 
    help = "Maxinm control mutation frquency", default = 1), make_option(c("--mapper_field"), 
    dest = "mapper_field", default = 3, help = "File name of which field is mapper name , eg. L01.exon.mapper.caller.txt, mapper field is 3"), 
  make_option(c("--caller_field"), dest = "caller_field", default = 4, help = "File name of which field is caller name , eg. L01.exon.mapper.caller.txt, caller field is 4"), 
  make_option(c("-o", "--outputcsv"), help = "output csv"), make_option(c("-s", 
    "--samplename"), help = "sample label"), make_option(c("-m", "--mode"), default = "somatic", 
...
if (opt$mode == "somatic") {
  ...
  postfil <- (filcasedepth & filcasefrq & (filcontrolfrq | !issame)) | (csv.out$Gene == 
    "FLT3" && str_detect(csv.out$Mutation.type, "frame"))
} else {
...
getmapcaller <- function(csv.out) {
  ...
  result <- NULL
  for (i in chrpos) {
    fil <- paste0(csv.out$Patient.ID, ":", csv.out$Chr, ":", csv.out$Start) == 
      i
    tmp <- csv.out[fil, ]
    mapper = NULL
...
fil2anaysisIndel <- (is.na(csv.out$caseMutationFrq) | (csv.out$caseMutationFrq == 
  0)) & (str_detect(csv.out$Base.change, "-") | str_length(csv.out$Base.change) > 
  3)
...
write.table(csv.out.indel, paste0(as.character(opt$outputcsv), ".del"), sep = "\t", 
...
write.table(csv.out, as.character(opt$outputcsv), sep = "\t", col.names = T, row.names = F, 
  quote = F)
