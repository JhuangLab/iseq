...
option_list <- list(make_option(c("-v", "--verbose"), action = "store_true", default = TRUE, 
  help = "Print extra output [default]"), make_option(c("-i", "--input"), help = "input fmt result file"), 
  make_option(c("-a", "--casesnv"), help = "input case snv frq file"), make_option(c("-b", 
    "--caseindel"), help = "input case indel frq file"), make_option(c("-c", 
    "--controlsnv"), default = "", help = "input control snv frq file"), make_option(c("-d", 
    "--controlindel"), default = "", help = "input control indel frq file"), 
...
fil <- str_detect(result$Func.refGene, "splicing|exonic") & (is.na(result$Mutation.type) | 
  result$Mutation.type == "NA" | str_detect(result$Mutation.type, "nonsynonymous|frame|stop|missense|nonsense|unknow"))
exon.out <- result[fil, ]
