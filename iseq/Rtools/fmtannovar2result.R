...
csv.out <- data.frame(csv.out[1:(Ref.col-1)], Base.change, csv.out[,(Alt.col + 1):ncol(csv.out)])
...
replace_gene_fun <- function(x,y) {return(str_replace_all(x, paste0(y, ":"), ""))}
for(i in 1:nrow(csv.out)) {
  csv.out$Amino.acid.change[i] <- replace_gene_fun(csv.out$Amino.acid.change[i], 
                                                                    csv.out$Gene[i])
}
##change Mutation type
csv.out$Mutation.type = str_replace_all(csv.out$Mutation.type,"nonsynonymous SNV","missense")
